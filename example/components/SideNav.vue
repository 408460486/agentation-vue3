...
const isForensic = ref(false)
const npmVersion = ref<string | null>(null)
const hasEntered = ref(false)
const forensicPerkKey = ref(0)
const prevForensic = ref(false)
const showBunny = ref(false)
const skipAnimation = ref(false)

// Module-level flag simulation
...
const links = computed(() => [
  ...
  { href: "/output", label: t('nav.output') },
  { href: "/api", label: t('nav.api'), badge: t('nav.new') },
  { href: "/changelog", label: t('nav.changelog') },
  { href: "/faq", label: t('nav.faq') },
])

const text = "/agentation vue3"
const delays = [0.1, 0.4, 0.48, 0.56, 0.64, 0.72, 0.8, 0.88, 0.96, 1.04, 1.12, 1.2, 1.28, 1.36, 1.44, 1.52, 1.6]
...
const earLeftClass = computed(() => {
  if (!hasEntered.value) return 'nav-ear-left-enter'
  return `nav-ear-left-idle${isForensic.value && route.path === '/output' ? ' forensic-perk' : ''}`
})

const earRightClass = computed(() => {
  if (!hasEntered.value) return 'nav-ear-right-enter'
  return `nav-ear-right-idle${isForensic.value && route.path === '/output' ? ' forensic-perk' : ''}`
})
...
onMounted(() => {
  ...
  onUnmounted(() => {
    window.removeEventListener('agentation-format-change', handleFormatChange as EventListener)
  })
})

// Watch for forensic perk animation
watch(() => isForensic.value, (newVal) => {
  if (hasEntered.value && newVal && !prevForensic.value) {
    forensicPerkKey.value++
  }
  prevForensic.value = newVal
})
</script>
...
<template>
  <nav class="side-nav">
    ...
    <div class="side-nav-logo">
      <div
        v-if="showBunny"
        :class="['bunny-slide-container', { 'skip-animation': skipAnimation }]"
      >
        <NuxtLink
          to="/"
          :style="{
        ...
        >
          ...
          <svg
            ...
            :style="{
              transform: isForensic && route.path === '/output' ? 'scale(1.15)' : 'scale(1)',
          ...
          >
            ...
            <!-- Circle eyes -->
            <circle
              class="nav-eye-left-idle"
              cx="8.277"
              cy="20.466"
              r="1.8"
              :fill="bunnyColor"
              :style="{ opacity: isForensic && route.path === '/output' ? 0 : undefined, transition: 'fill 0.2s ease-out, opacity 0.2s ease-out' }"
            />
            <circle
              class="nav-eye-right-idle"
              cx="19.878"
              cy="20.466"
              r="1.8"
              :fill="bunnyColor"
              :style="{ opacity: isForensic && route.path === '/output' ? 0 : undefined, transition: 'fill 0.2s ease-out, opacity 0.2s ease-out' }"
            />
            <!-- X eyes for forensic mode -->
            <g :stroke="bunnyColor" stroke-width="1.5" stroke-linecap="round" :style="{ opacity: isForensic && route.path === '/output' ? 1 : 0, transition: 'opacity 0.2s ease-out, stroke 0.2s ease-out' }">
      ...
      <div>
        <span
          ...
          :key="i"
          :class="skipAnimation ? '' : (i === 0 ? 'typed-slash' : 'typed-char')"
          :style="{
            color: i === 0 ? '#4C74FF' : 'inherit',
            animationDelay: skipAnimation ? '0s' : (i === 0 ? '0s' : `${delays[i]}s`),
          }"
        ><template v-if="char === ' '">&nbsp;</template><template v-else>{{ char }}</template></span>
      </div>
    </div>
    ...
    <div class="nav-meta">
      ...
      <div class="nav-meta-row">
        <span class="nav-meta-label">React</span>
        <a
      ...
      <!-- Vue3 -->
      <div class="nav-meta-row">
        <span class="nav-meta-label">Vue3</span>
        <a
          href="https://github.com/408460486/agentation-vue3"
          target="_blank"
          rel="noopener noreferrer"
          class="nav-meta-link"
          aria-label="GitHub"
      ...
      <!-- Vue2 -->
      <div class="nav-meta-row">
        <span class="nav-meta-label">Vue2</span>
        <a
          href="https://github.com/408460486/agentation-vue2"
          target="_blank"
          rel="noopener noreferrer"
          class="nav-meta-link"
          aria-label="GitHub"
