...
const baseDelays = [0.1, 0.4, 0.48, 0.54, 0.62, 0.7, 1.0, 1.08, 1.14, 1.22, 1.3]
const delays = baseDelays.map(d => d + bunnyEntranceTime)
...
const earLeftClass = computed(() => {
  if (!hasEntered.value) return 'mobile-ear-left-enter'
  return `mobile-ear-left-idle${isForensic.value && route.path === '/output' ? ' forensic-perk' : ''}`
})

const earRightClass = computed(() => {
  if (!hasEntered.value) return 'mobile-ear-right-enter'
  return `mobile-ear-right-idle${isForensic.value && route.path === '/output' ? ' forensic-perk' : ''}`
})
  ...
  <nav class="mobile-nav">
    ...
    <div class="mobile-nav-header">
      ...
      <div class="mobile-typed-logo">
        <div :class="['mobile-bunny-container', { show: showBunny }]">
          <svg
            v-if="showBunny"
            width="28"
            height="28"
    ...
    <!-- Navigation Links -->
    <div :class="['mobile-nav-links', { open: isOpen }]">
