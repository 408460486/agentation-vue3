// =============================================================================
// Icons - Vue 3 Functional Components
// =============================================================================

import { h, type FunctionalComponent, type CSSProperties } from 'vue'

interface IconProps {
  size?: number
}

interface IconPropsWithStyle extends IconProps {
  style?: CSSProperties
}

interface IconPropsWithColor extends IconProps {
  color?: string
}

interface IconCopyAnimatedProps extends IconProps {
  copied?: boolean
}

interface IconEyeAnimatedProps extends IconProps {
  isOpen?: boolean
}

interface IconPausePlayAnimatedProps extends IconProps {
  isPaused?: boolean
}
...
IconCheck.props = ['size']

// Checkbox checkmark (smaller, optimized for checkboxes)
export const IconCheckSmall: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 14
  return h('svg', { width: size, height: size, viewBox: '0 0 14 14', fill: 'none' }, [
    h('path', {
...
IconCheckSmall.props = ['size']

// List with sparkle icon
export const IconListSparkle: FunctionalComponent<IconPropsWithStyle> = (props) => {
  const size = props.size ?? 24
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none', style: props.style }, [
...
IconCheckSmallAnimated.props = ['size']

// Copy icon (two overlapping rectangles)
export const IconCopyAlt: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
...
IconCopyAlt.props = ['size']

// Animated copy/checkmark icon
export const IconCopyAnimated: FunctionalComponent<IconCopyAnimatedProps> = (props) => {
  const size = props.size ?? 24
  ...
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    ...
    h('g', { class: 'check-icon', style: { opacity: copied ? 1 : 0, transform: copied ? 'scale(1)' : 'scale(0.8)', transformOrigin: 'center' } }, [
      h('path', { d: 'M12 20C7.58172 20 4 16.4182 4 12C4 7.58172 7.58172 4 12 4C16.4182 4 20 7.58172 20 12C20 16.4182 16.4182 20 12 20Z', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }),
      h('path', { d: 'M15 10L11 14.25L9.25 12.25', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
    ])
  ])
}
IconCopyAnimated.props = ['size', 'copied']

// Eye icon (original)
export const IconEye: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M4.91516 12.7108C4.63794 12.2883 4.63705 11.7565 4.91242 11.3328C5.84146 9.9033 8.30909 6.74994 12 6.74994C15.6909 6.74994 18.1585 9.9033 19.0876 11.3328C19.3629 11.7565 19.3621 12.2883 19.0848 12.7108C18.1537 14.13 15.6873 17.2499 12 17.2499C8.31272 17.2499 5.8463 14.13 4.91516 12.7108Z', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }),
    h('path', { d: 'M12 14.25C13.2426 14.25 14.25 13.2426 14.25 12C14.25 10.7574 13.2426 9.75 12 9.75C10.7574 9.75 9.75 10.7574 9.75 12C9.75 13.2426 10.7574 14.25 12 14.25Z', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
  ])
}
IconEye.props = ['size']

// Eye icon (alt version - larger pupil)
export const IconEyeAlt: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 24
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
...
IconEyeAlt.props = ['size']

// Eye closed (with slash)
export const IconEyeClosed: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 24
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
...
IconEyeClosed.props = ['size']

// Animated eye icon that transitions between open/closed states
export const IconEyeAnimated: FunctionalComponent<IconEyeAnimatedProps> = (props) => {
  const size = props.size ?? 24
...
IconEyeAnimated.props = ['size', 'isOpen']

// Animated pause/play icon
export const IconPausePlayAnimated: FunctionalComponent<IconPausePlayAnimatedProps> = (props) => {
  const size = props.size ?? 24
  const isPaused = props.isPaused ?? false
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('style', {}, `.pause-bar, .play-triangle { transition: opacity 0.15s ease; }`),
    // Pause bars
    h('path', { class: 'pause-bar', d: 'M8 6L8 18', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', style: { opacity: isPaused ? 0 : 1 } }),
    h('path', { class: 'pause-bar', d: 'M16 18L16 6', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', style: { opacity: isPaused ? 0 : 1 } }),
    // Play triangle
    h('path', { class: 'play-triangle', d: 'M17.75 10.701C18.75 11.2783 18.75 12.7217 17.75 13.299L8.75 18.4952C7.75 19.0725 6.5 18.3509 6.5 17.1962L6.5 6.80384C6.5 5.64914 7.75 4.92746 8.75 5.50481L17.75 10.701Z', stroke: 'currentColor', 'stroke-width': '1.5', style: { opacity: isPaused ? 1 : 0 } })
  ])
}
IconPausePlayAnimated.props = ['size', 'isPaused']
...
export const IconPause: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 24
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M8 6L8 18', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round' }),
    h('path', { d: 'M16 18L16 6', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round' })
  ])
}
IconPause.props = ['size']

// Play icon
export const IconPlayAlt: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M17.75 10.701C18.75 11.2783 18.75 12.7217 17.75 13.299L8.75 18.4952C7.75 19.0725 6.5 18.3509 6.5 17.1962L6.5 6.80384C6.5 5.64914 7.75 4.92746 8.75 5.50481L17.75 10.701Z', stroke: 'currentColor', 'stroke-width': '1.5' })
  ])
}
IconPlayAlt.props = ['size']

// Trash can icon
export const IconTrashAlt: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M13.5 4C14.7426 4 15.75 5.00736 15.75 6.25V7H18.5C18.9142 7 19.25 7.33579 19.25 7.75C19.25 8.16421 18.9142 8.5 18.5 8.5H17.9678L17.6328 16.2217C17.61 16.7475 17.5912 17.1861 17.5469 17.543C17.5015 17.9087 17.4225 18.2506 17.2461 18.5723C16.9747 19.0671 16.5579 19.4671 16.0518 19.7168C15.7227 19.8791 15.3772 19.9422 15.0098 19.9717C14.6514 20.0004 14.2126 20 13.6865 20H10.3135C9.78735 20 9.34856 20.0004 8.99023 19.9717C8.62278 19.9422 8.27729 19.8791 7.94824 19.7168C7.44205 19.4671 7.02532 19.0671 6.75391 18.5723C6.57751 18.2506 6.49853 17.9087 6.45312 17.543C6.40883 17.1861 6.39005 16.7475 6.36719 16.2217L6.03223 8.5H5.5C5.08579 8.5 4.75 8.16421 4.75 7.75C4.75 7.33579 5.08579 7 5.5 7H8.25V6.25C8.25 5.00736 9.25736 4 10.5 4H13.5ZM7.86621 16.1562C7.89013 16.7063 7.90624 17.0751 7.94141 17.3584C7.97545 17.6326 8.02151 17.7644 8.06934 17.8516C8.19271 18.0763 8.38239 18.2577 8.6123 18.3711C8.70153 18.4151 8.83504 18.4545 9.11035 18.4766C9.39482 18.4994 9.76335 18.5 10.3135 18.5H13.6865C14.2367 18.5 14.6052 18.4994 14.8896 18.4766C15.165 18.4545 15.2985 18.4151 15.3877 18.3711C15.6176 18.2577 15.8073 18.0763 15.9307 17.8516C15.9785 17.7644 16.0245 17.6326 16.0586 17.3584C16.0938 17.0751 16.1099 16.7063 16.1338 16.1562L16.4668 8.5H7.5332L7.86621 16.1562ZM9.97656 10.75C10.3906 10.7371 10.7371 11.0626 10.75 11.4766L10.875 15.4766C10.8879 15.8906 10.5624 16.2371 10.1484 16.25C9.73443 16.2629 9.38794 15.9374 9.375 15.5234L9.25 11.5234C9.23706 11.1094 9.56255 10.7629 9.97656 10.75ZM14.0244 10.75C14.4384 10.7635 14.7635 11.1105 14.75 11.5244L14.6201 15.5244C14.6066 15.9384 14.2596 16.2634 13.8457 16.25C13.4317 16.2365 13.1067 15.8896 13.1201 15.4756L13.251 11.4756C13.2645 11.0617 13.6105 10.7366 14.0244 10.75ZM10.5 5.5C10.0858 5.5 9.75 5.83579 9.75 6.25V7H14.25V6.25C14.25 5.83579 13.9142 5.5 13.5 5.5H10.5Z', fill: 'currentColor' })
  ])
}
IconTrashAlt.props = ['size']
...
export const IconMoon: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M21 12.79A9 9 0 1111.21 3a7 7 0 009.79 9.79z', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
  ])
}
IconMoon.props = ['size']

// Checkmark in circle icon
export const IconCheckmarkCircle: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 24
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M12 20C7.58172 20 4 16.4182 4 12C4 7.58172 7.58172 4 12 4C16.4182 4 20 7.58172 20 12C20 16.4182 16.4182 20 12 20Z', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }),
    h('path', { d: 'M15 10L11 14.25L9.25 12.25', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
...
IconCheckmarkCircle.props = ['size']

// X mark / close icon
export const IconXmark: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M16.25 16.25L7.75 7.75', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }),
    h('path', { d: 'M7.75 16.25L16.25 7.75', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
  ])
}
IconXmark.props = ['size']

// Large X mark for marker delete button
export const IconXmarkLarge: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 12
  return h('svg', { width: size, height: size, viewBox: '0 0 12 12', fill: 'none' }, [
    h('path', { d: 'M9 3L3 9M3 3L9 9', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
  ])
}
IconXmarkLarge.props = ['size']

// Eye with minus (hidden/collapsed state)
export const IconEyeMinus: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M4.91516 12.7108C4.63794 12.2883 4.63705 11.7565 4.91242 11.3328C5.84146 9.9033 8.30909 6.74994 12 6.74994C15.6909 6.74994 18.1585 9.9033 19.0876 11.3328C19.3629 11.7565 19.3621 12.2883 19.0848 12.7108C18.1537 14.13 15.6873 17.2499 12 17.2499C8.31272 17.2499 5.8463 14.13 4.91516 12.7108Z', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }),
...
IconEyeMinus.props = ['size']

// Pause icon (two vertical bars - original)
export const IconPauseAlt: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('path', { d: 'M9.25 5.75C9.80228 5.75 10.25 6.19772 10.25 6.75L10.25 17.25C10.25 17.8023 9.80228 18.25 9.25 18.25L6.75 18.25C6.19772 18.25 5.75 17.8023 5.75 17.25L5.75 6.75C5.75 6.19772 6.19772 5.75 6.75 5.75L9.25 5.75Z', stroke: 'currentColor', 'stroke-width': '1.5' }),
    h('path', { d: 'M17.25 5.75C17.8023 5.75 18.25 6.19772 18.25 6.75L18.25 17.25C18.25 17.8023 17.8023 18.25 17.25 18.25L14.75 18.25C14.1977 18.25 13.75 17.8023 13.75 17.25L13.75 6.75C13.75 6.19772 14.1977 5.75 14.75 5.75L17.25 5.75Z', stroke: 'currentColor', 'stroke-width': '1.5' })
  ])
}
IconPauseAlt.props = ['size']

// Chat bubble with ellipsis
export const IconChatEllipsis: FunctionalComponent<IconPropsWithStyle> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none', style: props.style }, [
    h('path', { d: 'M18.8875 19.25L19.6112 19.0533C19.6823 19.3148 19.6068 19.5943 19.4137 19.7844C19.2206 19.9746 18.9399 20.0457 18.6795 19.9706L18.8875 19.25ZM14.9631 18.244L15.263 18.9314L14.9631 18.244ZM18.0914 15.6309L17.4669 15.2156L18.0914 15.6309ZM4.75 11.8041H5.5C5.5 15.2664 8.39065 18.1081 12 18.1081V18.8581V19.6081C7.60123 19.6081 4 16.1334 4 11.8041H4.75ZM19.25 11.8041H18.5C18.5 8.34166 15.6094 5.5 12 5.5V4.75V4C16.3988 4 20 7.47476 20 11.8041H19.25ZM12 4.75V5.5C8.39065 5.5 5.5 8.34166 5.5 11.8041H4.75H4C4 7.47476 7.60123 4 12 4V4.75ZM18.0914 15.6309L17.4669 15.2156C18.1213 14.2315 18.5 13.0612 18.5 11.8041H19.25H20C20 13.3681 19.5276 14.8257 18.716 16.0462L18.0914 15.6309ZM18.8875 19.25L18.1638 19.4467L17.2953 16.2517L18.019 16.055L18.7428 15.8583L19.6112 19.0533L18.8875 19.25ZM12 18.8581V18.1081C12.9509 18.1081 13.8518 17.9105 14.6632 17.5565L14.9631 18.244L15.263 18.9314C14.2652 19.3667 13.1603 19.6081 12 19.6081V18.8581ZM15.3144 18.2188L15.5224 17.4982L19.0955 18.5294L18.8875 19.25L18.6795 19.9706L15.1064 18.9394L15.3144 18.2188ZM14.9631 18.244L14.6632 17.5565C14.925 17.4423 15.2286 17.4134 15.5224 17.4982L15.3144 18.2188L15.1064 18.9394C15.1677 18.957 15.223 18.9489 15.263 18.9314L14.9631 18.244ZM18.0914 15.6309L18.716 16.0462C18.7451 16.0024 18.7636 15.9351 18.7428 15.8583L18.019 16.055L17.2953 16.2517C17.1957 15.8853 17.2716 15.5093 17.4669 15.2156L18.0914 15.6309Z', fill: 'currentColor' }),
    h('circle', { cx: '15', cy: '11.75', r: '1', fill: 'currentColor' }),
    h('circle', { cx: '12', cy: '11.75', r: '1', fill: 'currentColor' }),
    h('circle', { cx: '9', cy: '11.75', r: '1', fill: 'currentColor' })
  ])
}
IconChatEllipsis.props = ['size', 'style']

// Checkmark icon
export const IconCheckmark: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('g', { 'clip-path': 'url(#clip0_2_45)' }, [
      h('path', { d: 'M16.25 8.75L10 15.25L7.25 12.25', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
    ]),
    h('defs', {}, [
      h('clipPath', { id: 'clip0_2_45' }, [
        h('rect', { width: '24', height: '24', fill: 'white' })
      ])
    ])
  ])
}
IconCheckmark.props = ['size']

// Large checkmark icon
export const IconCheckmarkLarge: FunctionalComponent<IconProps> = (props) => {
  const size = props.size ?? 16
  return h('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none' }, [
    h('g', { 'clip-path': 'url(#clip0_2_37)' }, [
      h('path', { d: 'M17.5962 7.75L9.42308 16.25L6.15385 12.6538', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })
    ]),
    h('defs', {}, [
      h('clipPath', { id: 'clip0_2_37' }, [
        h('rect', { width: '24', height: '24', fill: 'white' })
      ])
    ])
  ])
}
IconCheckmarkLarge.props = ['size']

// Animated Bunny mascot
export const AnimatedBunny: FunctionalComponent<IconPropsWithColor> = (props) => {
  const size = props.size ?? 20
  const color = props.color ?? '#4C74FF'
  return h('svg', { width: size, height: size, viewBox: '0 0 28 28', fill: 'none', xmlns: 'http://www.w3.org/2000/svg' }, [
    h('style', {}, `
    ...
    h('circle', { class: 'bunny-eye-left', cx: '8.277', cy: '20.466', r: '1.8', fill: color }),
    h('circle', { class: 'bunny-eye-right', cx: '19.878', cy: '20.466', r: '1.8', fill: color }),
    h('text', { class: 'bunny-happy-face', x: '14', y: '26', 'text-anchor': 'middle', 'font-size': '12', 'font-weight': 'bold', fill: color, 'font-family': 'system-ui, -apple-system, sans-serif' }, '˃ ᵕ ˂')
  ])
}
AnimatedBunny.props = ['size', 'color']
